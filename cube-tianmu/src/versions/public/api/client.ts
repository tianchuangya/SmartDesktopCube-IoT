...
export const api = {
  ...
  renameDevice(deviceId: string, device_name: string) {
    return request<null>(`/device/${encodeURIComponent(deviceId)}/rename`, {
      method: 'PUT',
  ...
  getHistory(deviceId: string, hours = 24, limit = 80) {
    return request<SensorData[]>(`/data/${encodeURIComponent(deviceId)}/history?hours=${hours}&limit=${limit}`)
  },

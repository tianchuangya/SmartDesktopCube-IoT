...
def make_body(mat):
    ...
    obj = join("smart_cube_shell_v22_body_rear_screw_holes", parts)
    # The rear body screw receivers need a final local re-cut after joining.
    # Keep the cutter small so it opens the boss center without touching the whole shell.
    for y, z in [(-60, 16), (60, 16), (-60, 106), (60, 106)]:
        diff(obj, cyl(f"rear_body_final_screw_hole_{y}_{z}", (54, y, z), 2.0, 9, axis="X"))
    bevel(obj, 0.55, 2)
    return obj


def make_front_bezel(mat):
    bezel = box("v22_front_bezel_full_face_outer", (-64, 0, 59), (5, 138, 118), mat)
...
def make_side_slider(mat):
    ...
    slider = join("smart_cube_shell_v22_side_simple_slider_vents", parts)
    for z in (62, 74):
...
def make_back_cover(mat):
    cover = box("v22_back_cover_simple_two_ports_vents", (59, 0, 59), (8, 138, 120), mat)
...
def make_clear_lens(mat):
    lens = box("smart_cube_shell_v22_top_clear_lens", (-18, 44, 118), (48, 36, 1.8), mat)
    bevel(lens, 0.25, 1)
    return lens
...
def main():
    clear_scene()
    body_mat = material("v22 body blue gray", (0.10, 0.24, 0.34, 1))
    bezel_mat = material("v22 front bezel black", (0.03, 0.03, 0.035, 1))
    side_mat = material("v22 side slider black", (0.03, 0.03, 0.035, 1))
    cover_mat = material("v22 rear gray", (0.55, 0.55, 0.50, 1))
    lens_mat = material("v22 clear lens preview", (0.55, 0.85, 1.0, 0.35))
    ...
    export_stl(body, "smart_cube_shell_v22_body_rear_screw_holes.stl")
    export_stl(front, "smart_cube_shell_v22_front_bezel_real_holes.stl")
    export_stl(side, "smart_cube_shell_v22_side_simple_slider_vents.stl")
    export_stl(back, "smart_cube_shell_v22_back_cover_real_holes.stl")
    export_stl(lens, "smart_cube_shell_v22_top_clear_lens.stl")
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(BASE / "smart_cube_shell_v22_rear_body_screw_holes_preview.blend"))

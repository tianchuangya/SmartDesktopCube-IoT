...
def make_body(mat):
    ...
    diff(outer, box("front_screen_deep_pocket", (-58, 0, 59), (12, 104, 82)))
    ...
    for idx, (y, z) in enumerate([(-52, 22), (52, 22), (-52, 96), (52, 96)]):
    ...
    parts += [
        ...
        box("dc_tray_inner_top_lip", (3, -13, 34), (90, 12, 5), mat),
        ...
        box("esp32_tray_inner_top_lip", (3, 13, 34), (90, 12, 5), mat),
    ...
    obj = join("smart_cube_shell_v15_body_simple_slot", parts)
    bevel(obj, 0.55, 2)
    return obj


def make_front_bezel(mat):
    bezel = box("v15_front_bezel_flat_outer", (-64, 0, 59), (5, 116, 90), mat)
    ...
    for idx, (y, z) in enumerate([(-52, 22), (52, 22), (-52, 96), (52, 96)]):
        diff(bezel, cyl(f"front_bezel_screw_clearance_{idx}", (-64, y, z), 2.2, 8, axis="X"))
    bevel(bezel, 0.35, 2)
    return bezel
...
def make_side_slider(mat):
    ...
    parts = [
        box("v15_side_slider_flat_strip", (-4, 73, 55), (94, 5, 64), mat),
        box("v15_side_slider_upper_tongue", (-4, 68, 86), (90, 5, 4), mat),
        box("v15_side_slider_lower_tongue", (-4, 68, 24), (90, 5, 4), mat),
    ]
    slider = join("smart_cube_shell_v15_side_simple_slider", parts)
    bevel(slider, 0.3, 1)
    return slider


def make_back_cover(mat):
    cover = box("v15_back_cover_simple_two_ports", (59, 0, 59), (8, 138, 120), mat)
    ...
    for i, (y, z) in enumerate([(-60, 16), (60, 16), (-60, 106), (60, 106)]):
        diff(cover, cyl(f"back_cover_screw_clearance_{i}", (59, y, z), 2.2, 12, axis="X"))
    bevel(cover, 0.45, 2)
    return cover


def make_clear_lens(mat):
    lens = box("smart_cube_shell_v15_top_clear_lens", (-18, 44, 118), (48, 36, 1.8), mat)
    bevel(lens, 0.25, 1)
    return lens
...
def main():
    clear_scene()
    body_mat = material("v15 body blue gray", (0.10, 0.24, 0.34, 1))
    bezel_mat = material("v15 front bezel black", (0.03, 0.03, 0.035, 1))
    side_mat = material("v15 side slider black", (0.03, 0.03, 0.035, 1))
    cover_mat = material("v15 rear gray", (0.55, 0.55, 0.50, 1))
    lens_mat = material("v15 clear lens preview", (0.55, 0.85, 1.0, 0.35))
    ...
    export_stl(body, "smart_cube_shell_v15_body_simple_slot.stl")
    export_stl(front, "smart_cube_shell_v15_front_bezel_4screw.stl")
    export_stl(side, "smart_cube_shell_v15_side_simple_slider.stl")
    export_stl(back, "smart_cube_shell_v15_back_cover_two_ports.stl")
    export_stl(lens, "smart_cube_shell_v15_top_clear_lens.stl")
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(BASE / "smart_cube_shell_v15_simple_slot_preview.blend"))

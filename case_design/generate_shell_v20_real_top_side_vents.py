...
def make_body(mat):
    ...
    obj = join("smart_cube_shell_v20_body_real_top_side_vents", parts)
    bevel(obj, 0.55, 2)
    return obj


def make_front_bezel(mat):
    bezel = box("v20_front_bezel_full_face_outer", (-64, 0, 59), (5, 138, 118), mat)
...
def make_side_slider(mat):
    ...
    slider = join("smart_cube_shell_v20_side_simple_slider_vents", parts)
    for z in (62, 74):
...
def make_back_cover(mat):
    cover = box("v20_back_cover_simple_two_ports_vents", (59, 0, 59), (8, 138, 120), mat)
...
def make_clear_lens(mat):
    lens = box("smart_cube_shell_v20_top_clear_lens", (-18, 44, 118), (48, 36, 1.8), mat)
    bevel(lens, 0.25, 1)
    return lens
...
def main():
    clear_scene()
    body_mat = material("v20 body blue gray", (0.10, 0.24, 0.34, 1))
    bezel_mat = material("v20 front bezel black", (0.03, 0.03, 0.035, 1))
    side_mat = material("v20 side slider black", (0.03, 0.03, 0.035, 1))
    cover_mat = material("v20 rear gray", (0.55, 0.55, 0.50, 1))
    lens_mat = material("v20 clear lens preview", (0.55, 0.85, 1.0, 0.35))
    ...
    export_stl(body, "smart_cube_shell_v20_body_real_top_side_vents.stl")
    export_stl(front, "smart_cube_shell_v20_front_bezel_full_face.stl")
    export_stl(side, "smart_cube_shell_v20_side_simple_slider_vents.stl")
    export_stl(back, "smart_cube_shell_v20_back_cover_two_ports_vents.stl")
    export_stl(lens, "smart_cube_shell_v20_top_clear_lens.stl")
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(BASE / "smart_cube_shell_v20_real_top_side_vents_preview.blend"))

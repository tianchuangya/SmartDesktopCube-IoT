...
def add_body_screw_socket(parts, mat, y, z, idx):
    # Build the rear screw receiver as a rectangular ring instead of a solid boss
    # with a boolean-drilled center. The central passage is naturally empty.
    parts.append(box(f"body_back_screw_ring_top_{idx}", (52, y, z + 5.0), (12, 11, 3), mat))
    parts.append(box(f"body_back_screw_ring_bottom_{idx}", (52, y, z - 5.0), (12, 11, 3), mat))
    parts.append(box(f"body_back_screw_ring_left_{idx}", (52, y - 5.0, z), (12, 3, 11), mat))
    parts.append(box(f"body_back_screw_ring_right_{idx}", (52, y + 5.0, z), (12, 3, 11), mat))
...
def make_body(mat):
    ...
    # Rear cover screw holes must pass through the rear wall before the ring bosses.
    for y, z in [(-60, 16), (60, 16), (-60, 106), (60, 106)]:
        diff(outer, cyl(f"rear_wall_through_screw_hole_{y}_{z}", (56, y, z), 2.2, 10, axis="X"))
    ...
    parts += [
        ...
        # Edge support beams tie the upper module deck into the outer frame.
        # They keep the center wire window but remove unsupported floating shelf spans.
        box("upper_deck_left_edge_support", (-8, -58, 61), (88, 6, 8), mat),
        box("upper_deck_right_edge_support", (-8, 58, 61), (88, 6, 8), mat),
        box("upper_deck_front_left_bridge", (-50, -28, 61), (6, 38, 8), mat),
        box("upper_deck_front_right_bridge", (-50, 28, 61), (6, 38, 8), mat),
        box("upper_deck_rear_left_bridge", (36, -28, 61), (6, 38, 8), mat),
        box("upper_deck_rear_right_bridge", (36, 28, 61), (6, 38, 8), mat),
    ...
    obj = join("smart_cube_shell_v25_body_supported_upper_deck", parts)
    bevel(obj, 0.55, 2)
    return obj


def make_front_bezel(mat):
    bezel = box("v25_front_bezel_full_face_outer", (-64, 0, 59), (5, 138, 118), mat)
...
def make_side_slider(mat):
    ...
    slider = join("smart_cube_shell_v25_side_simple_slider_vents", parts)
    for z in (62, 74):
...
def make_back_cover(mat):
    cover = box("v25_back_cover_simple_two_ports_vents", (59, 0, 59), (8, 138, 120), mat)
...
def make_clear_lens(mat):
    lens = box("smart_cube_shell_v25_top_clear_lens", (-18, 44, 118), (48, 36, 1.8), mat)
    bevel(lens, 0.25, 1)
    return lens
...
def main():
    clear_scene()
    body_mat = material("v25 body blue gray", (0.10, 0.24, 0.34, 1))
    bezel_mat = material("v25 front bezel black", (0.03, 0.03, 0.035, 1))
    side_mat = material("v25 side slider black", (0.03, 0.03, 0.035, 1))
    cover_mat = material("v25 rear gray", (0.55, 0.55, 0.50, 1))
    lens_mat = material("v25 clear lens preview", (0.55, 0.85, 1.0, 0.35))
    ...
    export_stl(body, "smart_cube_shell_v25_body_supported_upper_deck.stl")
    export_stl(front, "smart_cube_shell_v25_front_bezel_real_holes.stl")
    export_stl(side, "smart_cube_shell_v25_side_simple_slider_vents.stl")
    export_stl(back, "smart_cube_shell_v25_back_cover_real_holes.stl")
    export_stl(lens, "smart_cube_shell_v25_top_clear_lens.stl")
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(BASE / "smart_cube_shell_v25_supported_upper_deck_preview.blend"))
